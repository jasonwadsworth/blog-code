...
export const handler = async (event: APIGatewayProxyEventV2): Promise<APIGatewayProxyResultV2<HierarchyItem>> => {
    ...
    try {
        return await handler.getNode(event);
    }
    catch (e) {
...
export class Handler {
    ...
    public async getNode(event: APIGatewayProxyEventV2): Promise<HierarchyItem> {
        const id = event.pathParameters?.id;
        if (!id) {
            throw new StatusCodeError('Missing required id path parameter.', 404);
        }

        const item = await this.hierarchyRepository.getHierarchyItem(id);

        if (!item) {
            throw new StatusCodeError('Unable to locate node.', 404);
        }
